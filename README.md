...
![n8n Workflow Flowchart](screenshot.png)
...
- **OpenAI API** (GPT-4o / GPT-3.5 Engine Integration)
- **Gmail API** (OAuth2 Automated Dispatch)
- **Google Sheets API** (Distributed Ledger/Log Matrix)

## 📦 Deployment & Setup

1. Clone or download the `resume_sender_gmail.json` file from this repository.
...
Advanced data manipulation, response sanitization (stripping rogue LLM markdown blocks like ` ```json `), and array state indexing are handled via specialized JavaScript sandboxes. You can review the precise code snippets inside the `clean_code_blocks.js` file.

---
Developed with ❤️ by [Ahmet Cevdet Bülbül](https://github.com/ahmetcevdetb)